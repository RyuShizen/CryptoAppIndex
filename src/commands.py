...
async def coinbase(ctx):
    ...
    embed.add_field(name="🏆 Current Rank", value=f"#️⃣{number_to_emoji(rank_number_coinbase)} in Finance", inline=False)
...
async def cwallet(ctx):
    ...
    embed.add_field(name="🏆 Current Rank", value=f"#️⃣{rank_number_emoji} in Finance", inline=False)
    ...
    sentiment_evaluation = evaluate_sentiment(coinbase_tracker, wallet_tracker)  # Assuming wallet_tracker is also defined
    embed.add_field(name="⛅ Market Sentiment", value=sentiment_evaluation, inline=False)
    embed.add_field(name="📆 Last Updated", value=current_datetime_hour, inline=False)
